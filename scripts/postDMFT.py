	...
	def interpol(self,emin,emax,rom,broaden,dest_dir,sp=False):
		...
		if sp:
			...
			for i in range(int(len(Sig)/2)):
				...
				#spin down
				SigSpline = interpolate.splrep(om, Sig[i+int(len(Sig)/2)].real, k=1, s=0)
				Sig_tot_dn[i,:] += interpolate.splev(ommesh, SigSpline)
				SigSpline = interpolate.splrep(om, Sig[i+int(len(Sig)/2)].imag, k=1, s=0)
...
if __name__ == "__main__":
	...
	parser_bands.add_argument('-sp',action='store_true', help='Flag to plot spin-polarized band structure')

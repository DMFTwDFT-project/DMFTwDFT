...
def Create_atomd(params_atomd):
   ...
   for p in params_atomd.keys():
       if p=="n=" or p=="Ewindow=" or p=="Ex=" or p=="Ncentral=" or p=="Ep=" or p=="Eimp=":
          print >> f, p, '[',
          for elem in params_atomd[p]:
             print >> f, str(elem)+',',
          print >> f, ']'
       elif p=="CoulombF=": print >> f, p, '\"'+params_atomd[p]+'\"'
       else: print >> f, p, params_atomd[p]
   f.close()
   ...
   for ispin in range(nspin):
      for j,orbs in enumerate(cor_orbs):
         for orb in orbs:
            ...
            if TB.cor_idx[idx]>1: mat_row[idx2+ispin*norb,idx2+ispin*norb]=loc_idx
         loc_idx+=1
   ...
   for i in range(len(cor_at)):
      if len(cor_orb[i])>0:
         ...
         if it==0 and itt==0:
            ...
            print os.popen('cp Trans'+str(i+1)+'.dat '+dir_name+'Trans.dat').read()
            #cmd = 'rm status.*'
            #print os.popen(cmd).read()
...
#         pC['mu']=[float(mu-ed[i]+vdc[i]),]
         ...
         #print os.popen(cmd).read()
         # Running ctqmc
         print 'Running ---- qmc for atom', i, 'iteration : ', it, '-----'
         strftime("%a, %d %b %Y %H:%M:%S", gmtime())

         cmd = para_com+" "+p['path_bin']+pC['exe'][0]+" > nohup.out  2> error"+str(i+1)+".out || { echo 'Parallel run failed!'; exit 1; }"
         out, err = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
         print out #, err

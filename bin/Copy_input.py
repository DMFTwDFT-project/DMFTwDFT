...
if __name__=='__main__':
   ...
   parser.add_argument('-post', type=str, help='Copy files for dos or band structure calculation?', choices=['dos','bands'])
   ...
   if post=='dos':
      ...
      bandsfiles=['wannier90.chk','wannier90.eig','DMFT_mu.out','dmft_params.dat','INPUT.py','para_com.dat','SigMdc.out','SigMdc_dn.out']
      ...
      DFTfiles={'VASP':['OUTCAR','OSZICAR','POSCAR','POTCAR','KPOINTS','INCAR','WAVECAR','para_com.dat'],'Siesta':[str(structure)+'.out','para_com.dat','POSCAR']}
      Ldft=False
      for dft in DFTfiles:
      ...
      if Ldft==True:
         ...
         if os.path.exists(cpdr+'/DFT_mu.out'):
            ...
            if dft =='siesta':
               data = fi.read()
               Efermi = float(re.findall(r'Fermi\s=[\s0-9+-.]*',data)[0].split()[-1])  
               savetxt('./DFT_mu.out',array([EFermi]))
               fi.close()

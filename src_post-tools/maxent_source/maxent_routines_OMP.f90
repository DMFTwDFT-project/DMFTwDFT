...
SUBROUTINE MAXENT(a,rfac,alpha,temp,ker,sxt,axt,mm,dw,nt,nw,steps,iseed)
  ...
  ! locals
  REAL*8 :: da(-nw:nw),xt1(nt),dxtw(nt),dxt(nt),xt2(nt)
  INTEGER:: i,j1,j2,jj,acc,try,k
  REAL*8 :: am,eps,x
  REAL*8 :: dj1,dj2,x1,x2,wgt
  REAL*8 :: aa1,aa2,de,p,mm1,mm2,arat
  REAL*8 :: time_pr, time_mt, t1w, t2w, t3w
  INTEGER, allocatable :: rvals(:)
  INTEGER :: ii,TID, Nthreads, OMP_GET_THREAD_NUM, OMP_GET_NUM_PROCS
  
  Nthreads = OMP_GET_NUM_PROCS()
  allocate( rvals(2*Nthreads) )
  
  WRITE(*,'(A,I3)') 'Number of threads=', Nthreads

  !!$OMP PARALLEL DO PRIVATE(TID) SCHEDULE(STATIC)
  !do k=1,Nthreads
  !   TID = OMP_GET_THREAD_NUM()
  !   print *, 'Num=', Nthreads, 'I am ', TID
  !enddo
!!$OMP END PARALLEL DO
  ...
  DO i=1,steps
     ...
     call walltim(t2w)
     time_pr=time_pr+t2w-t1w

     
     ! number of Metroolis steps equal to the number of frequency points
     DO ii=1,(2*nw+1)/Nthreads
        dxt(:)=0
        CALL GetNuniqueRandomNumbers(rvals, 2*Nthreads, nw)
        !$OMP PARALLEL DO &
        !$OMP& SHARED(rvals,da,a,am,ker,eps,dw,x1,alpha,temp) &
        !$OMP& PRIVATE(TID,j1,j2,x,dj1,dj2,dxtw,x2,xt2,aa1,aa2,mm1,mm2,de,wgt,p) SCHEDULE(STATIC) &
        !$OMP& REDUCTION(+:try,acc,dxt)
        DO jj=1,Nthreads
           ...
           do     
              ...
              enddo
              ! need positive a(j2)+dj2, where dj2~-dj1
              CALL random_number(x)
        ...
        ENDDO
        !$OMP END PARALLEL DO
        xt1(:) = xt1(:) + dxt(:)   ! G(tau) after Nthreads steps
        x1 = sum(sxt*(axt-xt1)**2) ! current chi2
     ENDDO

     call walltim(t3w)
     time_mt=time_mt+t3w-t2w
     ...
     ENDIF
  ENDDO
  WRITE(*,'(A,f10.3,2x,A,f10.3)') 'time matmul=', time_pr,'time Metropolis=', time_mt
...
SUBROUTINE GetNuniqueRandomNumbers(rvals, rvn, nw)

...
!~  end function utility_string_to_coord
  ...
  function fermi(eps)
    ...
    fermi=0.0_dp
    if (eps>100.0_dp) then
      fermi=0.0_dp
    else if (eps<-100.0_dp) then
      fermi=1.0_dp
    else
      fermi=1.0_dp/(1.0_dp+exp(eps))
    endif
  end function fermi

  function scalprod(a,b,dim)

    use constants, only: dp, cmplx_0
    IMPLICIT NONE

    integer, intent(in)  :: dim
    complex(kind=dp), intent(in) :: a(dim), b(dim)
    complex(kind=dp) :: scalprod
    INTEGER ::  i
  ...
  subroutine orthonormalize_Hermitian_eigvec(dim,evalue,evector)
    ...
    use constants, only: dp, cmplx_0, eps5
    implicit none
    ...
    DO i=1,dim
      DO j=1,i-1
        IF (abs(evalue(j)-evalue(i)).LT.eps5 .AND. &
        abs(scalprod(conjg(evector(:,j)),evector(:,i),dim)).GT.eps5) THEN
           evector(:,i) = evector(:,i) - scalprod(conjg(evector(:,j)),evector(:,i),dim)/ &
           scalprod(conjg(evector(:,j)),evector(:,j),dim)*evector(:,j)
        ENDIF
      ENDDO
    ENDDO
  ...
  end subroutine orthonormalize_Hermitian_eigvec


  subroutine orthonormalize_eigvec(dim,evalue,evl,evr)
    ...
    DO i=1,dim
      DO j=1,i-1
        IF (abs(evalue(j)-evalue(i)).LT.eps5 .AND. &
           abs(scalprod(evl(j,:),evr(:,i),dim)).GT.eps5) THEN
           evr(:,i) = evr(:,i) - scalprod(evl(j,:),evr(:,i),dim)/ &
           scalprod(evl(j,:),evr(:,j),dim)*evr(:,j)
        ENDIF
      ENDDO
      DO j=1,i-1
        IF (abs(evalue(j)-evalue(i)).LT.eps5 .AND. &
            abs(scalprod(evl(i,:),evr(:,j),dim)).GT.eps5) THEN
            evl(i,:) = evl(i,:) - scalprod(evl(i,:),evr(:,j),dim)/ &
            scalprod(evl(j,:),evr(:,j),dim)*evl(j,:)
        ENDIF
      ENDDO
    ENDDO
      !Normalize eigenvectors
  ...
  end subroutine orthonormalize_eigvec

  subroutine SVD(mat, dim1, dim2, evalue, cz, cvdag)
    ...
    integer :: INFO
    real(kind=dp), intent(out) :: evalue(dim2)
    real(kind=dp) :: RWORK(1+5*dim1)
    complex(kind=dp) :: evector(dim1,dim2), WORK(1+4*dim1)
    complex(kind=dp), intent(out) :: cz(dim1,dim1),cvdag(dim2,dim2)
    complex(kind=dp), intent(inout) :: mat(dim1,dim2)

    evalue=0.0_dp;cz=cmplx_0;cvdag=cmplx_0
    call ZGESVD('A', 'A', dim1, dim2, mat, &
          dim1, evalue, cz, dim1, cvdag, dim2, WORK, &
          4*dim1, RWORK, INFO)
  ...
  !============================================================!
  subroutine EIGENVALH(mat, dim, eig)
    !============================================================!
    !                                                            !
    ...
    call ZHEEVD('N', 'U', dim, mat, dim, eig, work, 2*dim+dim**2, &
                rwork, 1+5*dim+2*dim**2, iwork, 3+5*dim, info)
    if (info < 0) then
      write (stdout, '(a,i3,a)') 'THE ', -info, &
        ' ARGUMENT OF ZHEEVD HAD AN ILLEGAL VALUE'
      call io_error('Error in EIGVALH')
    endif
    if (info > 0) then
  ...
  end subroutine EIGENVALH

  !============================================================!
  subroutine EIGENH(mat, dim, eig)
    !============================================================!
    !                                                            !
    !! Diagonalize the dim x dim  hermitian matrix 'mat' and
    ...
    call orthonormalize_Hermitian_eigvec(dim,eig,mat)

  end subroutine EIGENH

  !============================================================!
  subroutine EIGENVAL(mat, dim, eig)
    !============================================================!
    !                                                            !
    ...
    eig = cmplx_0; evl = cmplx_0; evr = cmplx_0; work = cmplx_0; rwork = 0.0_dp
    call ZGEEV('N', 'N', dim, mat, dim, eig, evl, dim, evr, dim, &
                work, 2*dim+dim**2, rwork, info)
    if (info < 0) then
      write (stdout, '(a,i3,a)') 'THE ', -info, &
        ' ARGUMENT OF ZGEEV HAD AN ILLEGAL VALUE'
      call io_error('Error in EIGVAL')
    endif
    if (info > 0) then
      write (stdout, '(i3,a)') info, ' EIGENVECTORS FAILED TO CONVERGE'
      call io_error('Error in EIGVAL')
  ...
  !============================================================!
  subroutine EIGEN(mat, dim, eig, evl, evr)
    !============================================================!
    !                                                            !
    !! Diagonalize the dim x dim  general matrix 'mat' and
    !! return the eigenvalues 'eig' and the eigenvectors 'evl','evr'
    !                                                            !
    !============================================================!
    ...
    eig = cmplx_0; evl = cmplx_0; evr = cmplx_0; work = cmplx_0; rwork = 0.0_dp
    call ZGEEV('V', 'V', dim, mat, dim, eig, evl, dim, evr, dim, &
                work, 2*dim+dim**2, rwork, info)
    if (info < 0) then
      write (stdout, '(a,i3,a)') 'THE ', -info, &
        ' ARGUMENT OF ZGEEV HAD AN ILLEGAL VALUE'
    ...
    call orthonormalize_eigvec(dim, eig, evl, evr)

  end subroutine EIGEN

  !============================================================!
  subroutine MAT_INV(mat, dim)
    !============================================================!
    !                                                            !
    !! Diagonalize the dim x dim  hermitian matrix 'mat' and
    !! return the eigenvalues 'eig' 
    ...
    integer            :: i,j,info,ipiv(dim)
    complex(kind=dp) :: id(dim, dim)
    ...
    call ZGESV(dim,dim,mat,dim,ipiv,id,dim,info)

    if (info < 0) then
      write (stdout, '(a,i3,a)') 'THE ', -info, &
        ' ARGUMENT OF ZGESV HAD AN ILLEGAL VALUE'
      call io_error('Error in MAT_INV')
    endif
    if (info > 0) then
      write (stdout, '(i3,a)') info, ' MAT INV FAILED TO CONVERGE'
      call io_error('Error in MAT_INV')
    endif

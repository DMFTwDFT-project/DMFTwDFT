...
!          kz=2*pi*(2*c3-qz-1)/(2*qz)
     ...
     1      ,MPI_SUM,MPI_Comm_World,ierr)
      ...
      allocate(Gk(nom,num_wann,nkpts,nspin))
      allocate(tot_Gk(nom,num_wann,nkpts,nspin))
     ...
     1             num_wann,INFO)
              do i=1,num_wann
                Gk(im,i,nkp,ispin)=Gk(im,i,nkp,ispin)+id(i,i)
              enddo
            enddo
          enddo
        endif    
      enddo
      tot_Gk=0;
      CALL MPI_REDUCE(Gk,tot_Gk,nom*num_wann*nkpts*nspin,
     1      MPI_DOUBLE_COMPLEX,MPI_SUM,0,MPI_Comm_World,ierr)
      ...
      if (rank.eq.0) then
        ...
        allocate(write_Gloc(3,nom,num_wann,nkpts,nspin))
        do ispin=1,nspin
          do ik=1,nkpts
          do i=1,num_wann
            do im=1,nom
              write_Gloc(1,im,i,ik,ispin)=SigMdc(1,im,ispin)
              !do i=1,num_wann !ncor_orb
              write_Gloc(2,im,i,ik,ispin)=real(tot_Gk(im,i,ik,ispin))
              write_Gloc(3,im,i,ik,ispin)=aimag(tot_Gk(im,i,ik,ispin
     1          ))
              !enddo
            enddo
          enddo
        ...
        DO j=1,num_wann
          WRITE(99,*) 'orb=',j
        DO im=1,nom
          WRITE(99,'(3F18.12)') (write_Gloc(i,im,j,ik,1),i=1,3)
        ENDDO
        ENDDO
        ENDDO
        CLOSE(99)
        if (nspin.eq.2) then
          ...
          WRITE(99,*) 'orb=',j
          DO im=1,nom
            WRITE(99,'(3F18.12)') (write_Gloc(i,im,j,ik,2),i=1,3)
          ENDDO
          ENDDO
          ENDDO
          CLOSE(99)
